...
#define tricore_fdump(FIL, ...) tricore_vdump (FIL, __FUNCTION__, __VA_ARGS__)

extern int tricore_vdump (FILE*, const char*, ...);
extern void tricore_log_set_tricore_log (void);

typedef struct
{
  unsigned address_cost :1;
  unsigned canonicalize_comparison :1;
  unsigned clone :1;
  unsigned dam :1;
  unsigned debug :1;
  unsigned eabi :1;
  unsigned function_arg :1;
  unsigned legitimate_address_p :1;
  unsigned legitimize_address :1;
  unsigned mov_input_operand:1;
  unsigned must_load_indirect_p:1;
  unsigned pic:1;
...
extern tricore_log_t tricore_log;
