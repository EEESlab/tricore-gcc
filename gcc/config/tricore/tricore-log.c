/* Subroutines for log output for Tricore  back end.
...
/* This file supplies some functions for Tricore back-end developers
   with a printf-like interface.  The functions are called through
   macros `tricore_dump', `tricore_edump' or `tricore_fdump' from tricore-protos.h:

   tricore_fdump (FILE *stream, const char *fmt, ...);
   tricore_edump (fmt, ...) is a shortcut for tricore_fdump (stderr, fmt, ...)
   tricore_dump (fmt, ...)  is a shortcut for tricore_fdump (dump_file, fmt, ...)
...
tricore_log_t tricore_log;

/* The worker function implementing the %-codes */
static void tricore_log_vadump (FILE*, const char*, va_list);

/* Wrapper for tricore_log_vadump.  If STREAM is NULL we are called by tricore_dump,
   i.e. output to dump_file if available.  The 2nd argument is __FUNCTION__.
   The 3rd argument is the format string. */

int
tricore_vdump (FILE *stream, const char *caller, ...)
{
  va_list ap;
  ...
  va_start (ap, caller);
  if (stream)
    tricore_log_vadump (stream, caller, ap);
  va_end (ap);
...
static void
tricore_log_vadump (FILE *file, const char *caller, va_list ap)
{
  char bs[3] = {'\\', '?', '\0'};
      ...
      switch (*fmt++)
        ...
        case '%':
          switch (*fmt++)
            ...
            case '?':
              tricore_vdump (file, caller, "%F[%f:%P]");
              break;
...
void
tricore_log_set_tricore_log (void)
{
  bool all = TARGET_ALL_DEBUG != 0;

  if (all)
    tricore_log_details = "all";
	
  if (all || tricore_log_details)
    {
      /* Adding , at beginning and end of string makes searching easier.  */

      char *str = (char*) alloca (3 + strlen (tricore_log_details));
      bool info;

      str[0] = ',';
      strcat (stpcpy (str+1, tricore_log_details), ",");
...
#define SET_DUMP_DETAIL(S)                                       \
      do {                                                       \
	tricore_log.S = (all || strstr (str, "," #S ",") != NULL);   \
        if (info)                                                \
          fprintf (stderr, #S ",");                              \
      } while (0)
